...
#### 新增
- **内容审计报告** (`docs/content-audit-v8.0.md`)：记录被删除内容题材与角色信息
- **同人作品收藏指南** (`docs/fanwork-collection.md`)：41+条目，NGA/Pixiv/17173/角色站分类
- **新同人内容数据** (`docs/new-content-v8.0.json`)：17篇高质量同人内容
- **内容更新脚本** (`scripts/update-v8.0.py`)：自动化HTML/JS内容替换

#### 修改 — index.html
- **动态时间线**：6条 → 5条
  - 新增达妮娅SVG像素头像（白发粉紫渐变+泡泡元素）
  - 新增西格莉卡SVG像素头像（绿色+罗伊族花型头饰）
  - 新增漂泊者SVG像素头像（银发琥珀眼+黑海岸风格）
  - 角色覆盖：达妮娅×2 / 西格莉卡×1 / 漂泊者×1 / 飞行雪绒×1
- **角色日志**：3篇 → 4篇（空荡荡/第三个符文/她说是生日/娅娅）

#### 修改 — js/main.js
- **SEED_COMMENTS**：33条全部重写，非官方NPC → 官方角色
- **SEED_SUBMISSIONS**：6条 → 8条（覆盖四角色×五类型）
