...
function main() {
    ...
    /* 构建分流：档案向 type:lore 不进讨论区种子，避免 ensureCloudSeed / 本地合并污染 forum_submissions */
    const discussion = [];
    const loreOnly = [];
    all.forEach(function (item) {
        if (item && String(item.type || '').toLowerCase() === 'lore') loreOnly.push(item);
    ...
    const out = '// AUTO-GENERATED by scripts/build-forum-import.js — 请勿手改\n'
        ...
        + '// 分流：type:lore 档案向内容不写入 StarTorchImportSeed（见 CONTENT-PIPELINE）\n'
        + 'window.StarTorchImportSeed = ' + JSON.stringify(discussion, null, 0) + ';\n'
        + 'window.StarTorchLoreArchiveMeta = { excludedLoreCount: ' + loreOnly.length + ' };\n';

    fs.writeFileSync(OUT_FILE, out, 'utf8');
    console.log('\n✅ 讨论区种子 ' + discussion.length + ' 条（已排除 lore ' + loreOnly.length + ' 条）→ ' + path.relative(process.cwd(), OUT_FILE));
}
