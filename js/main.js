...
(function() {
    ...
    /* v11: 固定暗色主题（星空氛围）——light/auto 模式已移除 */
    function applyTheme() {
    ...
    var SEED_SUBMISSIONS = [
        ...
        },
        {
            ...
            realm: 'flying-edelweiss', tags: ['爱弥斯', '飞行雪绒'],
        ...
        /* —— 星炬学院主论坛（startorch）—— 由 forum/ 独立展示，不在飞行雪绒站渲染 —— */
        {
    ...
    function belongsToFlyingEdelweiss(s) {
        ...
        if (s.tags && Array.isArray(s.tags)) {
            for (var i = 0; i < s.tags.length; i++) {
                var tag = s.tags[i];
                if (tag === '爱弥斯' || tag === '飞行雪绒') return true;
            }
        }
        var hay = (s.title || '') + ' ' + (s.content || '');
        if (/飞行雪绒|爱弥斯|9072|调频9072/.test(hay)) return true;
        return false;
    }

    function _renderCommunityGrid(grid, empty, countEl, submissions) {
        // 仅渲染归属飞行雪绒论坛的内容
        submissions = submissions.filter(belongsToFlyingEdelweiss);
...
/* ============================================================
   模态框可访问性已抽至 js/modal-a11y.js（主站 / 论坛共用）
   ============================================================ */
